...
针对已出现的公式渲染遗漏，后续新增/改写文章必须遵守：

1. **块公式只用 `$$...$$`**
   - 不再使用 `\[ ... \]`，避免在当前 Markdown 渲染链路中被当作普通转义。

2. **行内公式统一用 `$...$`**
   - 统一分隔符，减少渲染分支差异。

3. **严禁引入隐藏控制字符**
   - 特别是在英文翻译或脚本替换后，检查是否出现不可见控制字符（会导致 `\tau`、`\text`、`\big`、`\nexists` 等命令损坏）。

4. **公式命令保真检查**
   - 发布前抽检关键命令是否完整：`\mathcal`、`\lambda`、`\mu`、`\tau`、`\text{}`、`\big()` 等。

5. **发布前双语渲染验收**
   - 中文与英文页面都要实际打开检查（不是只看 markdown 文本）。
   - 发现一侧异常，必须同步修两侧版本并重新发布。
